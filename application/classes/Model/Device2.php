...
/**
*добавлена функция readkey_onceInArray - вычитка карт из контроллера в массив
*добавлена функция getCardListIDX
*
*/

class Model_Device2 extends Model
{
	...
	public function readkey_onceInArray($id_dev)// 16.04.2024 вычитка карт из указанного контроллера в массив
	{
		$result=array();
			...
			//$value=$id_dev;
				...
				// готовлю соединение с ТС
						...
						if (false == ($socket = @socket_create(AF_INET, SOCK_STREAM, SOL_TCP))) {
							...
							$this->errors="Couldn't create socket, error code is: " . socket_last_error() .
									",error message is: " . socket_strerror(socket_last_error());
							return 3;// выход с сообщение об ошибке открытия сокета
							
							//HTTP::redirect("Couldn't create socket, error code is: " . socket_last_error().",error message is: " . socket_strerror(socket_last_error()));
						...
						if ($connection === false){
							...
							$this->errors='Cannot connect to TS server '.$ip_server .':'. $port.' in readkey_onceInArray function.';
							return 3;// выход с сообщение об ошибке открытия сокета
				...
				while ($i<$max_cell) //делаю выборку
						{
							...
							while ($res_command == 'ERR' and $i_count<10)// до 10 попыток чтения данных из ячейки
							{
								...
								//echo Debug::vars('145', $temp);
								// проверка результата ответ
								//echo Debug::vars('147',stripos($temp, 'OK') !== false );
								if(stripos($temp, 'OK') !== false)  $res_command = 'YES'; //если получили OK , то выходим из цикла
								//$res_command=substr($temp, 0, stripos($temp, " "));// выборка результат выполнения команды: OK или ERR
								$i_count++;
							}
							//echo Debug::vars('165', $reply);
							$key=str_replace(" ",'',Model::Factory('Stat')->parser_2($res1));
							parse_str(str_replace(",","&",$key), $bbb);
							$key2=substr($key, 0, stripos($key, ","));
							
							
							$mytext =str_replace(",",";",trim($key)); // строка данных
							
							$temp=explode(";", $mytext);//получил ответ как массив
							if(stripos(Model::Factory('Stat')->parser_1(Arr::get($temp, 2)), 'Yes') !== false ){
								$result[]=strtoupper(str_pad(Model::Factory('Stat')->parser_1(Arr::get($temp, 1)), 8, '0', STR_PAD_LEFT));
							}
					...
					$this->errors=__('readkey_once_is_not_door');
					return 3;
		...
		{
			$this->errors=__('readkey_once_is_not_number');
			return 3;
		...
		//echo Debug::vars('197',$result); exit;
	...
	public function getCardListIDX($id_dev)//выборка количества карт для указанной точки прохода, полученные из таблицы cardidx
	{
		$result=array();
		$sql='select cd.id_card from cardidx cd where cd.id_dev='.$id_dev.'
		order by cd.id_card';
	//echo Debug::vars('1210',$sql); exit;	
	try
			{
			$query = DB::query(Database::SELECT, $sql)
				->execute(Database::instance('fb'))
				...
				foreach($query as $key=>$value)
					{
						...
						//$result[]=Arr::get($value, 'ID_CARD');
						$result[]=strtoupper(str_pad(Arr::get($value, 'ID_CARD'), 8, '0', STR_PAD_LEFT));
					}
